...
<%
    ...
    boolean pUsuariosCrear = permisosUsuario != null && permisosUsuario.contains("personal.administracion.usuarios.crear");
    boolean pUsuariosListar = permisosUsuario != null && permisosUsuario.contains("personal.administracion.usuarios.listar");
    boolean pPerfilesCrear = permisosUsuario != null && (permisosUsuario.contains("seguridad.administracion.perfiles.crear") || permisosUsuario.contains("personal.administracion.perfiles.crear"));
    boolean pPerfilesAsignar = permisosUsuario != null && permisosUsuario.contains("seguridad.administracion.perfiles.asignar");
    ...
    boolean rUsuariosGenerar = permisosUsuario != null && permisosUsuario.contains("personal.reportes.usuarios.generar");
    boolean rMatriculasGenerar = permisosUsuario != null && permisosUsuario.contains("academico.reportes.matriculas.generar");
    boolean rFinancierosGenerar = permisosUsuario != null && permisosUsuario.contains("finanzas.reportes.financieros.generar");
%>
<!DOCTYPE html>
<html>
<head>
...
<div class="wrapper">
  ...
  <header class="main-header">
    ...
    <nav class="navbar navbar-static-top" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 0; display: flex; align-items: center; justify-content: space-between; box-shadow: 0 3px 12px rgba(102,126,234,.2);">
      <a href="#" class="sidebar-toggle" data-toggle="push-menu" role="button" style="color: white; padding: 15px 15px; transition: all .3s;">
        <span class="sr-only">Toggle navigation</span>
        <i class="fa fa-bars"></i>
      </a>
      
      <ul class="nav navbar-nav" style="margin: 0; padding: 0; display: flex; list-style: none; flex: 1;">
        <!-- Archivo Dropdown -->
        <li class="nav-item dropdown">
          <a href="#" style="color: white; padding: 15px 18px; display: block; transition: all .3s ease; font-weight: 500; font-size: 13px; border-bottom: 3px solid transparent;">
            <i class="fa fa-folder" style="margin-right: 8px;"></i> Archivo
          </a>
        ...
        <!-- Administración Dropdown (solo si tiene permisos) -->
        <% if (pUsuariosCrear || pUsuariosListar || pPerfilesCrear || pPerfilesAsignar) { %>
        <li class="nav-item dropdown">
          <a href="#" style="color: white; padding: 15px 18px; display: block; transition: all .3s ease; font-weight: 500; font-size: 13px; border-bottom: 3px solid transparent;">
            <i class="fa fa-cogs" style="margin-right: 8px;"></i> Administración
          </a>
          <ul class="dropdown-menu">
            <% if (pUsuariosCrear) { %>
        ...
        <!-- Procesos Dropdown (solo si tiene permisos) -->
        <% if (procMatriculasInscribir || procCalificacionesIngresar || procHorariosGestionar) { %>
        <li class="nav-item dropdown">
          <a href="#" style="color: white; padding: 15px 18px; display: block; transition: all .3s ease; font-weight: 500; font-size: 13px; border-bottom: 3px solid transparent;">
            <i class="fa fa-tasks" style="margin-right: 8px;"></i> Procesos
          </a>
          <ul class="dropdown-menu">
            <% if (procMatriculasInscribir) { %>
            <li><a href="matricula.jsp"><i class="fa fa-book" style="margin-right: 10px; width: 16px;"></i> Inscripción de Matrículas</a></li>
            <% } %>
            <% if (procCalificacionesIngresar) { %>
        ...
        <!-- Reportes Dropdown (solo si tiene permisos) -->
        <% if (rUsuariosGenerar || rMatriculasGenerar || rFinancierosGenerar) { %>
        <li class="nav-item dropdown">
          <a href="#" style="color: white; padding: 15px 18px; display: block; transition: all .3s ease; font-weight: 500; font-size: 13px; border-bottom: 3px solid transparent;">
            <i class="fa fa-bar-chart" style="margin-right: 8px;"></i> Reportes
          </a>
          <ul class="dropdown-menu" style="min-width: 280px;">
            <% if (rUsuariosGenerar) { %>
            <li><a href="srvUsuario?accion=exportarCsv"><i class="fa fa-file-excel-o" style="margin-right: 10px; width: 16px;"></i> Descargar Usuarios (CSV)</a></li>
            <li><a href="srvUsuario?accion=exportarPdf"><i class="fa fa-file-pdf-o" style="margin-right: 10px; width: 16px;"></i> Descargar Usuarios (PDF)</a></li>
            <% } %>
      ...
      <ul class="nav navbar-nav" style="margin: 0; padding: 0; display: flex; list-style: none; margin-left: auto;">
        <li class="nav-item dropdown">
          <a href="#" style="color: white; padding: 15px 18px; display: block; transition: all .3s ease; font-weight: 500; font-size: 13px; border-bottom: 3px solid transparent;">
            <i class="fa fa-user-circle" style="margin-right: 8px;"></i> ${sessionScope.nombreCompleto}
          </a>
          <ul class="dropdown-menu" style="right: 0; left: auto;">
            <li><a href="srvUsuario?accion=verPerfil&id=${sessionScope.idUsuario}"><i class="fa fa-user" style="margin-right: 10px; width: 16px;"></i> Mi Perfil</a></li>
            <li><a href="cambiarContrasena.jsp"><i class="fa fa-key" style="margin-right: 10px; width: 16px;"></i> Cambiar Contraseña</a></li>
            <li style="border-top: 1px solid rgba(255,255,255,.1); margin: 8px 0;"></li>
            <li><a href="srvUsuario?accion=logout"><i class="fa fa-sign-out" style="margin-right: 10px; width: 16px;"></i> Cerrar Sesión</a></li>
          </ul>
        </li>
      </ul>
    </nav>
...
<script>
  // Configurar comportamiento del navbar dinámico
  $(document).ready(function() {
    // Prevenir que los links se abran como nuevas páginas
    $('.nav-item.dropdown > a').click(function(e) {
      e.preventDefault();
      e.stopPropagation();
      $(this).parent().toggleClass('open');
      $('.nav-item.dropdown').not($(this).parent()).removeClass('open');
    });
    
    // Cerrar dropdown al hacer click fuera
    $(document).click(function(e) {
      if (!$(e.target).closest('.nav-item.dropdown').length && !$(e.target).is('.nav-item.dropdown > a')) {
